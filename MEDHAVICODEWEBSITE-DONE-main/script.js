...
const el = {
  ...
  loadSub:     $("#signinSub"),
  loadSeal:    $("#signinSeal"),
  loadLayers:  Array.from(document.querySelectorAll("#signinScene [data-speed]"))
};
...
const LOAD_DRIVE_MS   = 4200;   // truck drives through the scenery
const LOAD_CONFIRM_MS = 800;    // "Authenticated" is held on screen
const LOAD_FADE_MS    = 300;    // fade in, and fade into the portal
/* total from clicking Sign in ≈ 6.3s */

/* Scenery tuning */
const ROAD_SPEED_PX_S = 260;    // how fast the road dashes scroll (px/second)
const ENTER_END = 0.22;         // truck drives in to the centre until 22% ...
const EXIT_START = 0.80;        // ... cruises with the world scrolling, then leaves at 80%
const WHEEL_DEG_S = 900;        // wheel spin speed (degrees/second)
...
// slide every scenery strip; the sun has no data-speed so it never moves
function paintScenery(offset) {
  for (const layer of el.loadLayers) {
    const factor = parseFloat(layer.dataset.speed) || 0;
    layer.style.backgroundPositionX = `${-(offset * factor).toFixed(1)}px`;
  }
}
...
function driveTruck() {
  return new Promise((resolve) => {
    ...
    // CSS parks the truck just off-screen left (left: -truckWidth)
    const centreX = trackWidth / 2 + truckWidth / 2;   // truck centred in the scene
    ...
    // reduced motion: truck parked in the middle, scenery still, progress only
    if (prefersReducedMotion) {
      el.loadTruck.style.transform = `translateX(${centreX}px)`;
    ...
    const frame = (now) => {
      const t  = Math.min((now - started) / LOAD_DRIVE_MS, 1);
      const dt = Math.min((now - last) / 1000, 0.05);   // clamp after tab switches
      last = now;

      let x;           // truck position on screen
      let worldSpeed;  // 0..1 — how fast the scenery scrolls

      if (t < ENTER_END) {
        // 1) drive in from the left; the "camera" catches up, so the world speeds up
        const u = t / ENTER_END;
        x = centreX * easeOutCubic(u);
        worldSpeed = easeInOutCubic(u);
      } else if (t < EXIT_START) {
        // 2) cruise: truck holds the centre (tiny drift), scenery streams past
        const u = (t - ENTER_END) / (EXIT_START - ENTER_END);
        x = centreX + Math.sin(u * Math.PI * 2) * 4;
        worldSpeed = 1;
      } else {
        // 3) pull away to the right; the world slows as the camera stops following
        const u = (t - EXIT_START) / (1 - EXIT_START);
        x = centreX + (endX - centreX) * easeInCubic(u);
        worldSpeed = 1 - easeOutCubic(u) * 0.85;
      }

      // the whole vehicle moves together: body, chassis and both tyres
      el.loadTruck.style.transform = `translateX(${x.toFixed(1)}px)`;

      worldOffset += ROAD_SPEED_PX_S * worldSpeed * dt;
      paintScenery(worldOffset);

      // the truck is always rolling over the ground, so the hubs never stop
      wheelAngle += WHEEL_DEG_S * dt;
      el.loadWheelB.style.transform = `rotate(${wheelAngle}deg)`;
      el.loadWheelF.style.transform = `rotate(${wheelAngle}deg)`;
